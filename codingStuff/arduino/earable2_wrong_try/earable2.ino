...
Protocentral_MLX90632::status myStatus;
TwoWire myWire(&sercom1, SDA, SCL);
...
void setup() {
  ...
  for (uint8_t i = 0; i < NUM_MLX_CHANNELS; i++) {
    ...
    mlxSensors[i]->begin(TCA9548A_I2C_ADDRESS, myWire, myStatus);
    if (myStatus != Protocentral_MLX90632::status::SENSOR_SUCCESS) {
      Serial.print("Temperature sensor communication failed on sensor: ");
      Serial.println(i);
      while (true);
    }
  }
...
void loop() {
  if (i2cFailed()) {
    ...
    for (uint8_t i = 0; i < NUM_MLX_CHANNELS; i++) {
      TCA9548A_ch(MLX_CHANNELS[i]);
      objectTemps[i] = mlxSensors[i]->getObjectTemp(myStatus); // Get the temperature of the object we're looking at in C
//      if (i == 0) {
...
uint8_t TCA9548A_ch(uint8_t channel) {
  if (channel < TCA9548A_MAX_CHANNELS) {
    myWire.beginTransmission(TCA9548A_I2C_ADDRESS);
    myWire.write(0x00);
