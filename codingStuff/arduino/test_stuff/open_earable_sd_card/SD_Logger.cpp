...
//    } else if (r_type == R_TYPE_INT) {
    Serial.println("before convert int");
    text += convert_int((int*)data);
//    }
    text += "\r\n";

    Serial.println("if: yes -> dump_to_sd()");
    if (text.length() + _index > LOGGER_BUFFER_SIZE) {
        dump_to_sd();
    }
    Serial.println("text.toCharacter");
    text.toCharArray(&(_buffer[_index]), text.length());
    Serial.println("modify _index length");
...
void SD_Logger::dump_to_sd() {
    Serial.println("into dump_to_sd()");
    if (_index == 0) return;
