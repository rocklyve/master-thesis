...
The code of the two studies is located in the Git directory in the folder "codingStuff/studyAnalysis", respectively in the folder "study1" and "study2".
...
This is called in the pipeline and all results are either output or the plots are saved in the "target" folder or output to the console.
...
At the beginning the class \texttt{AnalysisPipeline}, which sets as class attributes the data folders, target folders. 
...
The \texttt{TemperatureData} class also contains the function to smoothen the data, which averages and smoothes the data within a window-size of $120$.
This corresponds to smoothing the data from a window of about 15 seconds, which was chosen to be very small over a total study length of 80 minutes.
In addition, the class includes a function to plot the smoothed raw data (\texttt{plot\_raw\_data}).
After executing the constructor of the class \texttt{AnalysisPipeline}, followed by reading in all collected study data, the data are correctly read in and can be used for the analysis of the hypotheses.
...
The results of the method are then output to the console for further interpretation.
...
\]
was used to determine the statistical significance of the results. Here, $k$ is the amount of sensors, which is $6$.
For each sensor 
\[
    \texttt{stats.ttest\_ind(indoor\_var, outdoor\_var)}
\]
is calculated and for a p-value below "bonferroni\_alpha" the null hypothesis was rejected.
The results were also output to the console.
...
The correlations range from $-1$ to $1$, with the value $-1$ representing a perfect negative correlation and the value $1$ representing a high correlation.
...
Hypothesis 5 looks at the absolute relative temperature changes and compares them to the mean absolute motion of the subjects, collected from the IMU data.
...
\]
the relative changes per sensor per subject is calculated, which is then averaged per sensor.
...
Also stored per subject is the attribute \texttt{kubios\_data}, which stores important findings from the analysis with the "KubiosHRV Standard" software.
These were calculated using the txt file per subject and read manually.
The class \texttt{HRVData} has the method \texttt{print\_statistics()}, which outputs all the information about the ground truth, HRV measurement in the console, including the RMSSD, SDNN, LF/HF ratio, during the different phases.
To plot the raw data, the class \texttt{RawDataPlotter} was created.
...
\paragraph{Hypothesis 2}
Hypothesis 2 now considers the temperature changes of the individual stress tests.
For this purpose, the data set was limited to the phase of the stress tests. 
